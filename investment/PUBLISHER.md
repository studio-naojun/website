...
- generated discovery artifacts produced by `python scripts/build_discovery.py` (article metadata, root `sitemap.xml`, `feed.xml`, `robots.txt`, and the IndexNow key file);
- optionally `investment/state.json` for monthly;
- assets only when a deliberate design evolution is part of the publication.

Before opening the PR, run `python scripts/build_discovery.py`. The Discovery SEO Smoke check must pass with `python scripts/build_discovery.py --check`; do not hand-edit generated discovery blocks.
