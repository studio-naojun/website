-- Run only after the u2memo private archive schema and real comment import are complete.
-- Compare these results with the generated migration-report.json / u2memo_migration_run.json.

select
  count(*) as comments_total,
  count(*) filter (where wordpress_status = '1') as approved_comments_total,
  count(*) filter (where legacy_secret is true) as legacy_secret_comments_total,
...
from public.u2memo_legacy_comments;
...
-- Privacy invariant: the retained schema itself contains no commenter email/IP/User-Agent/PASS columns.
select column_name
from information_schema.columns
where table_schema = 'public'
  and table_name = 'u2memo_legacy_comments'
