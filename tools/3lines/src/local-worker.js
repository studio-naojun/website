...
async function loadEngine() {
  if (!enginePromise) {
    enginePromise = import(WEBLLM_URL).then(({ CreateMLCEngine }) => {
      ...
      return CreateMLCEngine(MODEL_ID, {
        ...
        initProgressCallback: (progress) => post('progress', { progress: progress?.text || '' }),
      });
    });
  }
...
function promptFor(style, slate) {
  const styleText = {
    ...
    easy: '意味を変えず、平易な言葉にする',
    faithful: '条件・否定・留保・書き手の立場を保つ',
  }[style] || '結論・中心主張を優先する';
  return `次の原文由来の候補だけを使い、${styleText}要約を作ってください。外部知識や事実確認は禁止です。思考過程は出力せず、次の形式を厳守してください。\n1. 1つ目\n2. 2つ目\n3. 3つ目\n備考: 重要な条件・例外がある時だけ1行。なければ備考行は省略。\n各項目は120文字以内。\n\n候補:\n${slate}`;
}

self.addEventListener('message', async (event) => {
  if (event.data?.type !== 'summarize') return;
  const { requestId, style, slate } = event.data;
  try {
    post('preparing', { requestId, modelId: MODEL_ID });
    const engine = await loadEngine();
    post('ready', { requestId, modelId: MODEL_ID });
    const response = await engine.chat.completions.create({
      messages: [
        { role: 'system', content: 'あなたは日本語の要約器です。外部知識を使わず、指定形式だけを返します。' },
